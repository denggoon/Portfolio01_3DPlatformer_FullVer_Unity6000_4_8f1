...
public class EnemyCannonBall : MonoBehaviour {
	...
	public void onJumping() {
		...
		isJumping = true;
		thisRigidBody.linearVelocity = new Vector3 (0.0F, -5.0F, 0.0F);
	...
	void Update()
	{
		...
		if(GetComponent<Rigidbody>().linearVelocity != Vector3.zero) //강체의 속도 벡터가 0이 아니면 
			transform.rotation  = Quaternion.LookRotation(GetComponent<Rigidbody>().linearVelocity, transform.up); //벡터 방향으로 발사체가 바라보도록 설정 
	...
	void OnTriggerEnter(Collider collider) //무언가와 충돌 
	{
		if (collider.gameObject.layer == LayerMask.NameToLayer ("Player")) { //플레이어인 경우
			...
			if (GameRuleManager.instance.playerHealth.isInvincible == false) {
				GameRuleManager.instance.playerMove.KnockBack (GetComponent<Rigidbody> ().linearVelocity.normalized, knockbackDist); //넉백 효과도 줌 
				...
				if (GameRuleManager.instance.playerMove.playerSkill != null) {
					if (GameRuleManager.instance.playerMove.playerSkill is PlayerSkill_Harden && GameRuleManager.instance.playerMove.playerSkill.isActivated == true) {
						thisRigidBody.linearVelocity = Vector3.Reflect (thisRigidBody.linearVelocity, (Vector3.right + Vector3.forward).normalized);
						return;
					}
				}
			}
